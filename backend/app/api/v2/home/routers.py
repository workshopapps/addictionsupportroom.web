...
def get_all_notes(db: Session = Depends(get_db),
    ...
    notes = crud.get_all_notes(db=db, user=current_user)
    return notes


@router.post("/notes/create", response_model=schemas.ShowNote)
def create_note(note: schemas.Note,
                db: Session = Depends(get_db),
                current_user: User = Depends(deps.get_current_user)):
    db_note = crud.create_note(db=db, note=note, user=current_user)
    return db_note
...
def get_all_notes_created_today(db: Session = Depends(get_db),
    ...
    notes = crud.get_all_notes_created_today(db=db, user=current_user)
    return notes
...
def get_specific_note(note_id: int,
                      db: Session = Depends(get_db),
                      current_user: User = Depends(deps.get_current_user)):
    note = crud.get_specific_note(db=db, note_id=note_id, user=current_user)
    return note
...
def delete_note(note_id: int,
                db: Session = Depends(get_db),
                current_user: User = Depends(deps.get_current_user)):
    note = crud.delete_note(db=db, note_id=note_id, user=current_user)
    return note
...
def update_note(note: schemas.Note,
    ...
    note = crud.update_note(db=db, note_id=note_id, note=note, user=current_user)
    return note
