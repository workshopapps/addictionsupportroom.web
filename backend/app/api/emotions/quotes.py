happy = [
	"Be happy and keep crushing your goals",
	"If you find serenity and happiness, some may be jealous. Be happy anyway",
	"The most important thing is to enjoy your life—to be happy—it's all that matters",
]
confused = [
	"You have come this far,we are so proud of your progress, the sky is your limit & I am certain your future self will thank you",
	"When something is gone; something better is coming",
	"Sometimes it takes an overwhelming breakdown to have an undeniable breakthrough",
]
anger = [
	"And if today all you did was hold yourself together, I'm proud of you",
	"Sometimes it's ok if the only thing you managed to do today is breathe",
	"Getting knocked down in life is a given. Getting up and moving forward is a choice, so cheer up and move on",
]
sober = [
	"Recovery is a journey, and that means there will be bumps along the way. It's about progress not perfection.",
	"One must not let oneself be overwhelmed by sadness,get up, dust yourself and go again",
	"Whatever you are going through, is preparing you for your next breakthrough",
]
defeated = [
	"It deoesn't matter how slow you go, as long as you don't stop, You are not alone",
	"Being fearless is having a lot of fears but you jump anyway, so give it another try, you got this",
	"Just trust yourself. You've survived a lot, and you'll survive whatever is coming",
]
